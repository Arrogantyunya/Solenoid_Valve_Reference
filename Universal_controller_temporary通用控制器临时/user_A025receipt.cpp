...
void Receive_A025(unsigned char * Judgement_Data, int Judgement_Length)//A025函数
{
    ...
    //--------------------------------------------------------
   	...
   	Receive_IsBroadcast = Judgement_Data[6];//得到是否广播指令
	...
	Serial.println(String("Check_Length")+String(Check_Length));
	if (Check_Length > 0)
	{
...
unsigned char Water_pressure1()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		{
			Water_valueData1[i] = float(analogRead(VIN1) * 1000) / 569;//这里是水压测量的引脚VIN1，模拟输入电压1
		}
...
unsigned char Water_pressure2()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		{
			Water_valueData2[i] = (float(analogRead(VIN2)) * 1000) / 569;//这里是水压测量的引脚VIN2，
		}
