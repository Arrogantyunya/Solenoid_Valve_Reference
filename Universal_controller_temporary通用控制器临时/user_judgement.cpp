...
unsigned char Judgement_function(unsigned char * Judgement_Data, int Judgement_Length, int CRC_Check_num)//判断函数
{
	...
	//if(Judgement_flag == 0)
	//{
	//	Serial.println("Judgement_flag = 0");
	//	Working_parameter_setting = EEPROM.read(0);
	//	if (Working_parameter_setting == 0x01)
	//	{
	//		Judgement_flag = 1;
	//		Serial.println("Judgement_flag = 1");
	//	}
	//}
	...
	////实际使用请注释
	////这里是输出是否全部设置完成的标志位的值
	//Serial.print("Working_parameter_setting = ");
	//Serial.println(Working_parameter_setting,HEX);
	//delay(1500);
	////------------------------------------------------
	...
	if(Judgement_Data[0] == 0xFE)
	{
		...
		{
		  	//CRC8校验位的判断
			if(Judgement_Data[Judgement_Length - 6] == CRC_Check_num)//CRC_Check_num
			{
				//判断数据长度
				...
				if (Judgement_Data[3] == Judgement_Length + 0x01 - 0x0B)
				{
					...
					if(Judgement_Data[4] == 0xC0 && Judgement_Data[5] == 0x02)
					{
						...
						else
						{
							...
							// Serial3.print("Frame_ID_error帧ID错误");
							delay(500);
	...
	else
	{
		...
		// Serial3.print("Frame_header_error帧头错误");
		delay(500);
		Serial.println("该条指令被无视");
	}
